function plot_concentrations_paper(simfile,nrow,batchs)
load([simfile '.mat']);
ci=1;
    ...
    try
        for i=1:7 % rows
            ...
            for row=1:nrow %coolumns
                ...
                for ib=batchs
                    x = data.age_h';
                    y1=data.batch(ib).y(:,id);e=y1;if id==1;e=e*0.1;else;e=e*0.2;end
                    
                    y = data.batch(ib).ysim(:,id);
                    maxy1=max([maxy1;y]);miny1=min([miny1;y]);maxx1=max([maxx1;x]);minx1=min([minx1;x]);
                    if data.batch(ib).istrain==1
                        experimentalPlot = errorbar(x,y1,e,...
                        'MarkerSize',2,'Color','#0080ff','Marker','o','LineStyle','none','Linewidth',1,'HandleVisibility','off');
                    
                        trainingPlot = plot(x,y,...
                            'Color','#0080ff','Marker','none','LineStyle','-','Linewidth',2,'HandleVisibility','off');
                ...
                for ib=batchs
                    ...
                    if data.batch(ib).istrain==1
                        ...
                        experimentalPlot = errorbar(x,y1,e,...
                        'MarkerSize',2,'Color','#2bbf5c','Marker','o','LineStyle','none','Linewidth',1,'HandleVisibility','off');
                        testPlot = plot(x,y,...
                            'Color','#2bbf5c','Marker','none','LineStyle','-','Linewidth',2,'HandleVisibility','off');
                ...
                if id==np
                    ...
                    if ~isempty(train)
                        ...
                        y1=data.batch(ib).y(:,id);e=y1;if id==1;e=e*0.1;else;e=e*0.2;end
                        experimentalPlot = errorbar(x,y1,e,...
                            'MarkerSize',2,'Color','#0080ff','Marker','o','LineStyle','none','Linewidth',1,'HandleVisibility','on');
                        y = data.batch(train(1)).ysim(:,id);
                        trainingPlot = plot(x,y,...
                            'Color','#0080ff','Marker','none','LineStyle','-','Linewidth',2,'HandleVisibility','on');
                    ...
                    if ~isempty(test)
                        ...
                        y1=data.batch(ib).y(:,id);e=y1;if id==1;e=e*0.1;else;e=e*0.2;end
                        experimentalPlot = errorbar(x,y1,e,...
                            'MarkerSize',2,'Color','#2bbf5c','Marker','o','LineStyle','none','Linewidth',1,'HandleVisibility','on');

                        y = data.batch(test(1)).ysim(:,id);st='on';
                        testPlot = plot(x,y,...
                            'Color','#2bbf5c','Marker','none','LineStyle','-','Linewidth',2,'HandleVisibility','on');
                    end
%                     if numel(il)>3;il=il(1:3);end
                    lg={'experimental' 'train'  'experimental'  'test'  };
                ...
                ylim(hAx1,[ 0 max([maxy1])*1.25])
                xlim(hAx1,[ 0 max([maxx1])*1.15])
                axis square
%                 if np-id>nrow-1
                    set(hAx1,'Units','centimeters','position',position,'Box','on','YMinorGrid','off','XMinorGrid','off','FontSize',8,'FontName','Arial');hold on;
%                 else
...
%                 end
                ...
                if id~=1
                    ylabel([Names{id}],'FontSize',8,'interpreter','tex','FontName','Arial','FontWeight','bold');
                else
                    ylabel([' ' Names{id} ' '],'FontSize',8,'interpreter','tex','FontName','Arial','FontWeight','bold');
                end
                xlabel('Time (h)','FontSize',8,'FontName','Arial','FontWeight','bold');
                ...
                text(min([ minx1])+xrange*0.75,min([miny1 ])+yrange*1.15,[lis((i)) num2str(row) '  '],'HorizontalAlignment','right','VerticalAlignment','top','FontSize',9,'FontName','Arial','FontWeight','bold')
